...
export default function QuizScreen({ question, questionNumber, totalQuestions, onNext, onMenu }: Props) {
  ...
  const handleSubmit = () => {
    if (!input.trim() && !input.replace(/　/g, "").trim()) return;
        ...
        <div className={styles.resultArea}>
          ...
          <div className={styles.answerBox}>
            <div className={styles.yourAnswer}>
              あなたの回答：<code>{input}</code>
            </div>
            {!isCorrect && (
              <div className={styles.correctAnswer}>
                正解：<code>{question.answer.join(" / ")}</code>
              </div>
            )}
          </div>
